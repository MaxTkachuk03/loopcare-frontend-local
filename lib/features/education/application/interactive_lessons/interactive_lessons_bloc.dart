...
class InteractiveLessonsBloc extends Bloc<InteractiveLessonsEvent, InteractiveLessonsState> {
  ...
  Future<void> _onGetMealTime(
    ...
    final mealsTime = mealsListItems.reversed
        .where((meals) => meals.mealCategory == state.data.mealCategory)
        .expand((meals) => meals.mealItems.map((meal) => meal.updatedAt.toLocal()))
        .toList();

    emit(InteractiveLessonsState.lessonLoaded(
      state.data.copyWith(
        mealsTime:
            mealsTime.length > 1 && state.data.mealCategory != MealCategory.inbetweens.originalValue
