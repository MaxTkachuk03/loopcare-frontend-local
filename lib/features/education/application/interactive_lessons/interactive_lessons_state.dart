...
class InteractiveLessonsStateData with _$InteractiveLessonsStateData {
  ...
  const factory InteractiveLessonsStateData({
    ...
    @Default({}) Map<String, RiverModuleItem> moduleItem,
    ...
    @Default([]) List<DateTime> mealsTime,
  ...
  bool get isAllComponentChecked {
    final componentsWithProgress = unlockedChunkComponents.where((c) {
      ...
      if (c is InteractiveLessonChunkComponentMealTiming) {
        return false;
      }
  ...
  bool hasProgress(List<InteractiveLessonChunkComponent> unlockedChunkComponents) {
    final quizComponents = unlockedChunkComponents
        .where((c) =>
            c.type != InteractiveLessonComponentType.image &&
            c.type != InteractiveLessonComponentType.markdown &&
            c.type != InteractiveLessonComponentType.mealTiming)
    ...
    return quizComponents.every((c) {
      if (c.type == InteractiveLessonComponentType.textArea) {
        if (c.progress == null || c.progress!.history == null) {
          return false;
        }
  ...
  List<InteractiveLessonChunkComponent> getChunkComponents(InteractiveLessonChunk chunk) {
    final chunkComponents = components.values
        .where((component) =>
