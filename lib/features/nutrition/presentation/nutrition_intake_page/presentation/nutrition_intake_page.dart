...
class NutritionIntakePage extends StatefulWidget {
  const NutritionIntakePage({super.key, @PathParam('selectedDay') required this.selectedDay});
...
class _NutritionIntakePageState extends State<NutritionIntakePage> {
  ...
  void _completeDay() {
    ...
    String convertedDate = cutTimeInDate(lessonDate);

    context.read<NutritionIntakeBloc>().add(NutritionIntakeEvent.completeDay(date: convertedDate));

    context.read<CommitmentBloc>().add(const CommitmentEvent.updateCommitment());
  }

  String cutTimeInDate(String lessonDate) {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<NutritionIntakeBloc, NutritionIntakeState>(
      ...
      builder: (context, nutritionState) {
        final isCompleted = nutritionState.data.progress.any((c) => c.isLessonFinished);

        return nutritionState.maybeWhen(
            error: (_) => const SizedBox.shrink(),
            loading: (_) => CustomScaffold.customColor(
                  color: AppColors.white,
                  body: const Center(child: CircularProgressIndicator()),
                ),
            orElse: () => BlocBuilder<MealsBloc, MealsState>(
                                            ...
                                            child: ListView.separated(
                                                ...
                                                itemBuilder: (context, index) {
                                                  final selectedDayMeals = mealsState.data.meals[
                                                      mealsState.data.currentDateTime
                                                  ...
                                                  final progress = nutritionState.data.progress;
                                                  return NutritionIntakeButton(
                                                    ...
                                                    lessonDate: cutTimeInDate(lessonDate),
                                                  );
                                                },
                                        ...
                                        isCompleted
                                            ? Column(
                                                ...
                                                children: [
                                                  ...
                                                  ContinueBtn(
                                                      label:
                                                          LocalizedTexts.backToPracticeBoard.tr(),
                                                      onPressed: () {
