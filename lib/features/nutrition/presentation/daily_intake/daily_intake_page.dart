                  ...
                  children: [
                    Expanded(
                      child: ListView.separated(
                        itemCount: state.filledCategories.length,
                        itemBuilder: (BuildContext context, index) {
                          final selectedDayMeals =
                              mealsState.meals[mealsState.currentDate?.isoStringWithoutTime];

                          if (selectedDayMeals != null) {
                            var category = selectedDayMeals[index].mealCategory;
                            var mealItems = selectedDayMeals[index].mealItems;

                            return MealCard(
                              mealId: selectedDayMeals[index].id,
                              title: category,
                              mealItems: mealItems,
                              calorieDensity: mealsState.calorieDensitySum(mealItems),
                            );
                          }
                        },
