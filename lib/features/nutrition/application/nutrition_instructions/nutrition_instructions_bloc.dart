...
@singleton
class NutritionInstructionsBloc extends Bloc<NutritionInstructionsEvent, NutritionInstructionsState> {
  final NutritionService nutritionService;

  NutritionInstructionsBloc(this.nutritionService) : super(const NutritionInstructionsState.initial()) {
  ...
  FutureOr<void> _onFetchValuesExplanation(
    ...
    response.fold(
      (l) => emit(NutritionInstructionsState.error(l)),
      (r) {
        final calorieDensityValues =
            r.data.where((el) => el.category == NutritionInstructionCategory.calorieDensity.name).toIList();
        final proteinDegreeValues =
            r.data.where((el) => el.category == NutritionInstructionCategory.proteinDegree.name).toIList();
