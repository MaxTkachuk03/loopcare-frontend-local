    ...
    switch (module.iconType.toString()) {
      case 'reflection':
        iconPath = module.states?.itemState == 'completed'
            ? AppIcons.tick
            : (module.states?.itemState != 'locked'
                ? AppIcons.account
      ...
      case 'nutrition':
        iconPath = module.states?.itemState == 'completed'
            ? AppIcons.tick
            : (module.states?.itemState != 'locked' ? AppIcons.spoons : AppIcons.foodUnlocked);
        break;
      case 'goal':
        iconPath = module.states?.itemState == 'completed'
            ? AppIcons.tick
            : (module.states?.itemState != 'locked' ? AppIcons.goalLocked : AppIcons.goalUnlocked);
        break;
      case 'weight':
        iconPath = module.states?.itemState == 'completed'
            ? AppIcons.tick
            : (module.states?.itemState != 'locked' ? AppIcons.vector : AppIcons.weightUnlocked);
        break;
      case 'commitment':
        iconPath = module.states?.itemState == 'completed'
            ? AppIcons.tick
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ListView.builder(
                  shrinkWrap: true,
                  primary: false,
                  itemCount: moduleItem.moduleItems!.length,
                  itemBuilder: (context, index) {
                    final module = moduleItem.moduleItems![index];

                    String text =
                        "${module.widgetStatus!.state.toString()} ${module.streamType.toString()}";
                    String iconPath = getIconPathForModule(module); // Use the refactored function
