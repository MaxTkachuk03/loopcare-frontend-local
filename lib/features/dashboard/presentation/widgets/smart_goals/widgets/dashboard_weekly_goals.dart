...
class _DashboardWeeklyGoalsState extends State<DashboardWeeklyGoals> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<SmartGoalsBloc, SmartGoalsState>(builder: (context, state) {
      return state.maybeMap(
          ...
          orElse: () {
            if (!state.data.hasWeeklyGoals || !state.data.hasReviewDelay) return const WeeklyGoalsEmptyState();
            return Padding(
                ...
                children: [
                  ...
                  if (state.data.hasQuickReviewWeeklyGoals)
                    CustomElevatedButton.greenSmall(
                      ...
                      onPressed: () => _onQuickReviewHandler(context),
                    ),
                ],
              ),
            );
  ...
  _onQuickReviewHandler(BuildContext context) {
    // push route Quick review
  }
}
